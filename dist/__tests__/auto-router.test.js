...
describe('autoRouter', () => {
    ...
    it('should warn via console when directory name is an HTTP method keyword', async () => {
        ...
        const mockApp = { get: jest.fn(), $routes: undefined };
    ...
    it('should reject file with named exports alongside default export', async () => {
        ...
        writeFileSync(join(namedDir, 'get-test.js'), 'export const foo = 1;\nexport default async (ctx) => {}');
        const mockApp = { get: jest.fn(), $routes: undefined };
        const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => { });
    ...
    it('should allow plain object with handler function in non-strict mode (with warning)', async () => {
        ...
        writeFileSync(join(objDir, 'get-test.js'), 'export default { handler: async (ctx) => { ctx.body = {} } }');
        const mockApp = { get: jest.fn(), $routes: undefined };
        const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => { });
    ...
    it('should warn when createHandler explicit meta overrides forceProtected pattern', async () => {
        ...
        writeFileSync(join(metaDir, 'get-open.js'), 'export default { handler: async (ctx) => {}, meta: { requiresAuth: false }, $__isRouteConfig: true }');
        const mockApp = { get: jest.fn(), $routes: undefined };
        const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => { });
        const logSpy = jest.spyOn(console, 'log').mockImplementation(() => { });
        await autoRouter({
    ...
    it('should warn when createHandler explicit meta overrides forcePublic pattern', async () => {
        ...
        writeFileSync(join(metaPubDir, 'get-secure.js'), 'export default { handler: async (ctx) => {}, meta: { requiresAuth: true }, $__isRouteConfig: true }');
        const mockApp = { get: jest.fn(), $routes: undefined };
        const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => { });
        const logSpy = jest.spyOn(console, 'log').mockImplementation(() => { });
        await autoRouter({
    ...
    it('should warn when forcePublic pattern never matches any registered route', async () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('forcePublic pattern "/api/nonexistent-route" did not match any registered route'));
        warnSpy.mockRestore();
        logSpy.mockRestore();
    ...
    it('should warn when forceProtected pattern never matches any registered route', async () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('forceProtected pattern "/api/nonexistent-route" did not match any registered route'));
        warnSpy.mockRestore();
        logSpy.mockRestore();
    ...
    it('should match forcePublic/forceProtected via method-prefix pattern', async () => {
        ...
        const mockApp = { get: jest.fn(), post: jest.fn(), $routes: undefined };
        const logSpy = jest.spyOn(console, 'log').mockImplementation(() => { });
        await autoRouter({
        ...
        expect(protectedRoutes.some((r) => r.method === 'POST' && r.path === '/api/users')).toBe(true);
        expect(publicRoutes.some((r) => r.method === 'GET' && r.path === '/api/users')).toBe(true);
        logSpy.mockRestore();
    ...
    it('should skip file silently when default export is null', async () => {
        ...
        await autoRouter({
            dir: nullDir,
            prefix: '/api',
            onLog: (level, msg) => { if (level === 'error')
                errors.push(msg); },
        })(mockApp);
        expect(mockApp.get).not.toHaveBeenCalled();
    ...
    it('should catch import error when module throws during evaluation', async () => {
        ...
        writeFileSync(join(throwDir, 'get-test.js'), "throw new Error('intentional module load error')");
        const mockApp = { get: jest.fn(), $routes: undefined };
        const errors = [];
        await autoRouter({
            dir: throwDir,
