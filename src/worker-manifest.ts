...
export type WorkerRouteHandler<TEnv = unknown, TCtx = unknown> = (
  ctx: WorkerRouteContext<TEnv, TCtx>,
...
export function createWorkerRouter<TEnv = unknown, TCtx = unknown>(options: WorkerRouterOptions<TEnv, TCtx>) {
  ...
  return {
    fetch: async (req: Request, env: TEnv, ctx: TCtx) => {
      try {
...
function createWorkerContext<TEnv, TCtx>(req: Request, env: TEnv, ctx: TCtx, params: Record<string, string>): WorkerRouteContext<TEnv, TCtx> {
  return {
    req,
    env,
    ctx,
...
function normalizeWorkerResponse(result: unknown) {
