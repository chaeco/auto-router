...
用方括号 `[]` 包裹参数名，会被转换为 Express 风格的 `:param` 路径段。**参数名保留原始大小写**——`[userId]` 注册为 `:userId`，`ctx.params` 的键名与你书写一致；`[userId]` 与 `[UserID]` 视为同一条路由（去重大小写不敏感）。
...
参数名（方括号内的内容）仅允许 **ASCII 字母、数字、下划线**（`[A-Za-z0-9_]`），并**保留原始大小写**——`[userId]` 注册为 `:userId`，`ctx.params.userId` 与你书写一致。去重**仅折叠大小写**：`get-[userId].ts` 与 `get-[UserID].ts` 视为同一条路由（二者匹配相同的 URL）；但 `user_id` 是**不同的参数名**——`get-[userId].ts` 与 `get-[user_id].ts` 注册为两条独立路由。以下写法会被**拒绝**并跳过该文件（同时打印错误日志），不会静默注册出坏路由：

| 非法写法 | 原因 |
|----------|------|
| `get-[].ts` | 空参数 |
| `get-[a][b].ts` | 相邻参数必须用 `-` 分隔，应为 `get-[a]-[b].ts` |
| `get-[id.ts` / `get-id].ts` | 括号不配对 |
...
> 注：`staticAutoRouter` 的 `path` 应直接使用 Express 风格 `:param`（如 `'/api/users/:id'`），文件名的 `[param]` 语法是文件命名约定，不适用于静态路由声明。
...
标准 `autoRouter` 依赖 `fs` + 动态 `import()`，这在边缘运行时（Cloudflare Workers、Deno Deploy 等）和打包后的 Node.js 应用中不可用。提供两种解决方案：

### staticAutoRouter

手动导入 handler 并以数据形式声明路由——适用于任意打包工具。
...
Cloudflare Workers 需要不同的方式：它们使用 Web Platform 的 `fetch(request, env, ctx)` 签名，而非框架中间件。使用**构建时 CLI** + **`createWorkerRouter`** 来支持 Workers。

#### 步骤 1：构建时生成清单

CLI 扫描控制器目录，生成包含导入和路由定义的静态 TypeScript 文件：
...
#### 步骤 2：在 Worker 中使用 `createWorkerRouter`
...
#### Workers 的 Handler 签名

Workers handler 接收单个 `WorkerRouteContext` 参数（不是 Express 的 `req, res`）：
...
interface WorkerRouteContext<TEnv = unknown, TCtx = ExecutionContext> {
  ...
  params: Record<string, string>        // 路由参数（:id、:userId 等，保留原始大小写）
...
Handler 可以通过三种方式返回数据（按顺序检查）：

1. **直接返回 `Response`** → 原样使用
2. **返回任意非 null/undefined 值** → 自动序列化为 JSON
3. **返回 `undefined`** → 序列化 `ctx.res` 构建器

```typescript
// 方式 1：直接返回 Response（完全控制）
export default async (ctx) => {
  return new Response('Hello', { status: 200, headers: { 'X-Custom': 'value' } })
}

// 方式 2：返回普通对象（自动转 JSON）
export default async (ctx) => {
  return { users: ['Alice', 'Bob'] }  // → JSON 响应，Content-Type: application/json
}

// 方式 3：修改 ctx.res（底层方式）
export default async (ctx) => {
  ctx.res.status = 201
  ctx.res.headers['X-Custom'] = 'value'
  ctx.res.body = 'Created'
  // 返回 undefined 触发 ctx.res 序列化
}
```
...
**`createWorkerRouter` 不集成权限解析**（`defaultRequiresAuth`、`forcePublic`、`forceProtected`）。Workers 的权限是应用层逻辑——handler 直接接收 `env`（包含 secrets/KV/D1），因此在 handler 内部或通过包装器实现权限逻辑：
...
#### 为什么不在 Workers 中使用 `staticAutoRouter`？
...
适用于 Cloudflare Workers 和其他使用 Web Platform `fetch(request, env, ctx)` 的运行时。返回带有 `fetch` 方法的路由器。

**选项：**

| 选项 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `routes` | `WorkerManifestRoute[]` | **必填** | 路由数组（来自生成的清单） |
| `notFound` | `(req, env, ctx) => Response \| Promise<Response>` | `404 'Not Found'` | 自定义 404 处理器 |
| `onError` | `(err, req, env, ctx) => Response \| Promise<Response>` | `500 'Internal Server Error'` | 自定义错误处理器 |

**`WorkerManifestRoute`：**

```typescript
interface WorkerManifestRoute<TEnv = unknown, TCtx = ExecutionContext> {
  pattern: string   // Express 风格路径：'/api/users/:id'
  method: string    // 'GET'、'POST' 等
  handler: unknown  // 路由 handler 函数或 createHandler 结果
}
```
