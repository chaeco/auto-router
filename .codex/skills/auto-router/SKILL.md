...
**Rule of thumb:** ≤3 path segments → flat file. >3 segments → nested directories. **Param names keep their casing** (`[userId]` → `:userId`, `ctx.params.userId`); duplicate detection is case-insensitive on param names. See [REFERENCE.md](REFERENCE.md) for the full conversion algorithm.
