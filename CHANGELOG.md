...
- **Build system → Rollup** — replaced the `tsc` emit pipeline with a unified `rollup` bundle. Main entry, `worker-manifest` subpath, and the `auto-router-build-manifest` CLI are each bundled to `dist/` (ESM + bundled `.d.ts` via `rollup-plugin-dts`). Relative source imports no longer need a `.js` extension; `moduleResolution` is now `bundler`.
