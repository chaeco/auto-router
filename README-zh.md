...
参数名（方括号内的内容）仅允许 **ASCII 字母、数字、下划线**（`[A-Za-z0-9_]`），并**保留原始大小写**——`[userId]` 注册为 `:userId`，`ctx.params.userId` 与你书写一致。去重对参数名大小写不敏感，`get-[userId].ts` 与 `get-[UserID].ts` 视为同一条路由。以下写法会被**拒绝**并跳过该文件（同时打印错误日志），不会静默注册出坏路由：
