 ...
 */
export function resolveAuth(options: AuthResolutionOptions): AuthResolutionResult {
...
export class ForcePatternTracker {
  readonly matchedForcePublicPatterns = new Set<string>()
  readonly matchedForceProtectedPatterns = new Set<string>()
  readonly overriddenByMeta: Array<{ route: string; pattern: string; type: 'forcePublic' | 'forceProtected' }> = []
  readonly conflictRoutes: Array<{ route: string; publicPattern: string; protectedPattern: string }> = []

  /** Record a forcePublic/forceProtected pattern match. */
  addMatch(publicPattern?: string, protectedPattern?: string): void {
    if (publicPattern) this.matchedForcePublicPatterns.add(publicPattern)
    if (protectedPattern) this.matchedForceProtectedPatterns.add(protectedPattern)
  }
